...
//        Arrays.sort(points);
        ...
        for (int i = 0; i < pointList.size(); i++) {
            if (pointList.get(i) instanceof Model.Point[]) {
                for (int j = 0; j < dateCount; j++) {
                    dateList.add(((Model.Point[]) pointList.get(i))[j].getExpireDate());
                }
            }
        }
        //③①のArrayList.sortメソッドで昇順にソート
        dateList.sort(new PointComparator());

        //④Object型ArrayListのDate型の要素にソート後の①を要素が小さい方から代入し直す
        for (int i = 0; i < dateCount; i++) {
            if (pointList.get(i) instanceof Model.Point[]) {
                for (int j = 0; j < dateCount; j++) {
                    ((Model.Point[]) pointList.get(i))[j].setExpireDate(dateList.get(j));
                }
//                pointList.set(i, dateList.get(i));
            }
        }
...
//            if (pointList.get(i) instanceof Model.Point) {
//                dateList.add(((Model.Point) pointList.get(i)).getExpireDate());
//            }
//        }
//        //③①のArrayList.sortメソッドで昇順にソート
//        dateList.sort(new PointComparator());
...
//            if (pointList.get(i) instanceof Model.Point) {
//                pointList.set(i, dateList.get(i - 6));
//            }
//        }
        for (int i = 0; i < pointList.size(); i++) {
...
class PointComparator implements Comparator {
    ...
    public int compare(Object o1, Object o2) {
        if (o1 instanceof String && o2 instanceof String) {
            ...
            String point1 = (String) o1;
            String point2 = (String) o2;
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd");
            try {
                Date formatDate1 = sdf.parse(point1);
                Date formatDate2 = sdf.parse(point2);
                return formatDate1.compareTo(formatDate2);
            } catch (ParseException e) {
                e.printStackTrace();
            }
